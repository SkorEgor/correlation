...
def correlation_renge(width_in_counts, middle_index, data_2, data_1):
    correlation_range = []
    half_of_window = width_in_counts // 2

    for m in range(middle_index - half_of_window, middle_index + half_of_window + 1):
        correlation_val = 0
        for n in range(middle_index - half_of_window, middle_index + half_of_window + 1):
            correlation_val += data_1[n] * data_2[n - m]
        correlation_range.append(correlation_val)
    return correlation_range


def correlation(width_in_counts, middle_index, data_the_standard, data):
    half_of_window = width_in_counts // 2

    correlation_val = 0
    for n in range(middle_index - half_of_window, middle_index + half_of_window + 1):
        correlation_val += data_the_standard[n] * data[n]
...
class GuiProgram(Ui_Dialog):
    ...
    def signal_difference(self):
        ...
        window_width_in_counts = int(self.lineEdit_2.text())
        if window_width_in_counts % 2 == 0: window_width_in_counts += 1

        # for i in range(window_width_in_counts // 2 + 1, len(self.empty_gamma) - window_width_in_counts // 2 ):
        #    val = correlation(window_width_in_counts, i, self.empty_gamma, self.signal_gamma)
        #    self.ax2.plot(self.empty_frequency[i - window_width_in_counts // 2:i + window_width_in_counts // 2+1], val, color='g', label='empty')

        self.difference = []
        for i in range(0, window_width_in_counts // 2 + 1):
            self.difference.append(0)
        for i in range(window_width_in_counts // 2 + 1, len(self.empty_gamma) - window_width_in_counts // 2):
            self.difference.append(correlation(window_width_in_counts, i, self.empty_gamma, self.signal_gamma))
            print(correlation(window_width_in_counts, i, self.empty_gamma, self.signal_gamma))
        for i in range(len(self.empty_gamma) - window_width_in_counts // 2, len(self.empty_gamma)):
            self.difference.append(0)
